    ...
    function yandex_get_value($arOffer, $param, $PROPERTY, &$arProperties, &$arUserTypeFormat)
    {
        ...
        if (isset($arProperties[$PROPERTY]) && !empty($arProperties[$PROPERTY]))
        {
                ...
                case 'E':
                    if (!empty($arProperty['VALUE']))
                    {
                        ...
                        if (!empty($arCheckValue))
                        {
                            ...
                            {
                                $value .= ($value ? ', ' : '').mb_strtolower($arRes['NAME'], 'UTF-8');
                            }
                        }
                    }
                    break;
            ...
            if ($bParam)
            {
                ...
                {
                    $strProperty .= '<param name="'.yandex_text2xml(mb_strtolower($arProperties[$PROPERTY]['NAME'], 'UTF-8'), true).'">'.yandex_text2xml($value, true).'</param>';
                }
            }
            else
            {
...
if (empty($arRunErrors))
{
    ...
    {
        $strTmpCat.= "<category id=\"".$value["ID"]."\"".(intval($value["IBLOCK_SECTION_ID"])>0?" parentId=\"".$value["IBLOCK_SECTION_ID"]."\"":"").">".yandex_text2xml(mb_strtolower($value["NAME"], 'UTF-8'), true)."</category>\n";
    }
    if (isset($value))
        unset($value);
...
/*                  $dbVAT = CCatalogProduct::GetVATInfo($arAcc['ID']);
            ...
            foreach ($arYandexFields as $key)
            {
                ...
                case 'name':
                    ...
                    $strTmpOff .= "<name>".yandex_text2xml(mb_strtolower($arAcc["NAME"], 'UTF-8'), true)."</name>\n";
                    break;
                case 'description':
                    $strTmpOff .=
                        "<description>".
                ...
                if (!empty($arCurrentOffer) && !empty($arCurrentPrice))
                {
                    ...
                    foreach ($arYandexFields as $key)
                    {
                        ...
                        case 'name':
                            ...
                            $strOfferYandex .= "<name>".yandex_text2xml(mb_strtolower($arOfferItem["NAME"], 'UTF-8'), true)."</name>\n";
                            break;
                        case 'description':
...
/*                      $dbVAT = CCatalogProduct::GetVATInfo($arItem['ID']);
                ...
                foreach ($arYandexFields as $key)
                {
                    ...
                    case 'name':
                        ...
                        $strValue = "<name>".yandex_text2xml(mb_strtolower($arItem["NAME"], 'UTF-8'), true)."</name>\n";
                        break;
                    case 'description':
                        $strValue =
                            "<description>".
