...
$(function(){
    $("form.profile-edit").submit(function(){
        ...
        console.log(email);
        $(this).find("input[name=LOGIN]").val(email);        
    });
})

</script>

<form method="post" name="form1" class="profile-edit" action="" enctype="multipart/form-data">
...
<?endif?>
    ...
    <input type="reset" class="button1" value="<?=GetMessage('MAIN_RESET');?>">
    <input type="submit" class="button2" name="save" value="<?=(($arResult["ID"]>0) ? GetMessage("MAIN_SAVE") : GetMessage("MAIN_ADD"))?>">
</form>
<?
