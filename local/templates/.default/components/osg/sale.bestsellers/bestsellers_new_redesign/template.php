...
<div class="cabinet-detail-title">БЕСТСЕЛЛЕРЫ</div>
    ...
    <?
        ...
        <?foreach ($arResult['ELEMENT'] as $key=>$arItem):?>
            ...
            <div class="bestseller-item"> 
                ...
                <input type="hidden" id="price<?=$arItem["ID"]?>" value="<?=ceil(CCurrencyRates::ConvertCurrency($arItem['PRICE']['PRICE'], $arItem['PRICE']['CURRENCY'], "RUR"))?>">
                ...
                <div class="best-item-code"><?=$arItem["CODE"]?></div>
                <div class="best-item-price"><?=ceil(CCurrencyRates::ConvertCurrency($arItem['PRICE']['PRICE'], $arItem['PRICE']['CURRENCY'], "RUR"))?> <font class="rouble">i</font></div>
                <div class="best-item-photo" title="<?=$arItem["NAME"]?>">
                    <a href="<?=$arItem['DETAIL_PAGE_URL']?>" class="bestseller_img">
                        <img src="<?=$img_path?>" class="rightpic" >
                    </a>                   
                </div>  
                <?
                    ...
                    if ($item_quantity["COUNT"] > 0) {
                        ...
                        ?>
                        <div class="add-item-basket">
                            <button class="button1" onclick="showcatdet('<?=$arItem["ID"]?>')">В КОРЗИНУ</button>
                        </div>                        
                        <?}
                } else {?>
                     <div class="add-item-basket">
                            <button class="button1" onclick="return false;">ОТСУТСТВУЕТ</button>
                     </div>  
                <?}?>
